...
const OrderService = {
    ...
    cancelOrder: async (obj) => {
        ...
        const orderToCancel = await Order.findOne({ user: userId, orderId: orderId })
        if (orderToCancel.orderStatus != ORDER_STATUS.DELIVERED)
            orderToCancel.orderStatus = ORDER_STATUS.CANCELED
        else
            throw new Error("Already order is delivered")
    ...
    updateOrder: async (obj) => {
        ...
        orderToUpdate.lastModifiedTime = moment(orderToUpdate.lastModifiedTime).subtract(2, 'hours');
        if ((moment(orderToUpdate.lastModifiedTime).diff(moment().format(), 'hours') * -1) < 1) {
            console.log("VALUE" + moment(orderToUpdate.lastModifiedTime).diff(moment().format(), 'hours'))
    ...
    getAllOrders: async (obj,query) => {
        return await Order.find({ user: obj }).skip(query.pagesize*(query.pageNumber-1)).limit(Number(query.pagesize))
    }, 
    getAllOrdersByResIds: async (obj) => {
        var d = obj.date.split("T")[0];
        return await Order.find({ restaurant: { $in: obj.list }, lastModifiedTime: { "$gte": new Date(d), $lt: new Date(moment(d).add(1, "days")) } }).skip(obj.pagesize*(obj.pageNumber-1)).limit(Number(obj.pagesize))

    }, getTotalAmountOnGivenDate: async (obj) => {
        var d = obj.date.split("T")[0];
        return await Order.find({ restaurant: { $in: obj.list }, lastModifiedTime: { "$gte": new Date(d), $lt: new Date(moment(d).add(1, "days")) } }, { totalPrice: 1, _id: 0 })
    },
